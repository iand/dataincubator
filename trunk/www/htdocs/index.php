...
class StoreBackedUriSpace extends PAGET_UriSpace {
  function get_description($uri) {
    if (preg_match("~oecd-glossary~", $uri)) {
      $desc = new StoreBackedResourceDescription($uri, 'http://api.talis.com/stores/iand-dev1'); 
      $desc->set_namespace_mapping('concept', 'http://dataincubator.org/oecd-glossary/concepts/');
      $desc->set_namespace_mapping('theme', 'http://dataincubator.org/oecd-glossary/themes/');
      $desc->set_namespace_mapping('segment', 'http://dataincubator.org/oecd-glossary/segments/');
      return $desc;
    }
  } 
